...
export default class SectionsContainer extends React.Component {
    ...
    componentDidMount() {
        ...
        this._handleResize('fromInit').then(() => {
            if (!this.props.scrollBar) {
                this._addCSS3Scroll();

                this._handleAnchor(true); //Go to anchor in case we found it in the URL
    ...
    _handleSectionTransition(index, fromInit) { 
        console.log(index)
        ...
        this.setState({
            scrollingStarted: fromInit ? false : true,
            activeSection: index,
            sectionScrolledPosition: position
        }, () => {
            this._resetScroll();
